...
resolve_vvip_relay_listen() {
  ...
  [[ -n "$relay_value" ]] || relay_value="off"
  case "$(lower "$relay_value")" in
    0|false|no|off) return 0 ;;
    auto)
      parse_host_port "$listen_value"
      local listen_host="$PARSED_HOST"
      local listen_port="$PARSED_PORT"
      if [[ "$listen_port" =~ ^[0-9]+$ && "$listen_port" -lt 65535 ]]; then
        format_endpoint "$listen_host" "$((listen_port + 1))"
      fi
      ;;
...
listen_port_requires_privileged_bind() {
  local port relay_listen relay_port
  ...
  if [[ -n "$relay_listen" ]]; then
    ...
    if [[ "$relay_port" =~ ^[0-9]+$ && "$relay_port" -lt 1024 ]]; then
      return 0
    fi
  fi
  return 1
...
configure_binary_bind_capability() {
  if listen_port_requires_privileged_bind; then
    command -v setcap >/dev/null 2>&1 || die "Listening on ports below 1024 requires setcap; please install libcap2-bin/libcap."
    log "Privileged port detected; granting cap_net_bind_service to server binary"
    setcap 'cap_net_bind_service=+ep' "$BIN_PATH" || die "Failed to set cap_net_bind_service on ${BIN_PATH}."
    return 0
  fi
...
if [[ -n "\$VVIP_RELAY_LISTEN" && ! "\$VVIP_RELAY_LISTEN" =~ ^(0|false|FALSE|no|NO|off|OFF)$ ]]; then
  if [[ "\$VVIP_RELAY_LISTEN" =~ ^(auto|AUTO)$ ]]; then
    listen_port="\${TOPFLOW_LISTEN##*:}"
    listen_host="\${TOPFLOW_LISTEN%:*}"
    if [[ "\$listen_port" =~ ^[0-9]+$ && "\$listen_port" -lt 65535 ]]; then
...
open_firewall_port() {
  ...
  if command -v ufw >/dev/null 2>&1 && ufw status 2>/dev/null | grep -qi 'Status: active'; then
    ...
    ufw allow "${port}/tcp" >/dev/null || warn "UFW allow failed; run manually: ufw allow ${port}/tcp"
    return 0
  fi
  if command -v firewall-cmd >/dev/null 2>&1 && systemctl is-active --quiet firewalld; then
    log "firewalld active; allowing ${port}/tcp"
    firewall-cmd --permanent --add-port="${port}/tcp" >/dev/null || warn "firewalld allow failed; run manually: firewall-cmd --permanent --add-port=${port}/tcp"
...
configure_firewall_add() {
  ...
  local port relay_listen relay_port opened="false"
  parse_host_port "$TOPFLOW_LISTEN"
  port="$PARSED_PORT"
  if [[ "$port" =~ ^[0-9]+$ ]]; then
    open_firewall_port "$port" && opened="true"
  else
    warn "Could not parse main listen port from $TOPFLOW_LISTEN; skipped firewall auto-open."
  fi
  relay_listen="$(resolve_vvip_relay_listen "$TOPFLOW_LISTEN" "$TOPFLOW_VVIP_RELAY_LISTEN")"
  if [[ -n "$relay_listen" ]]; then
    parse_host_port "$relay_listen"
    ...
    if [[ "$relay_port" =~ ^[0-9]+$ ]]; then
      open_firewall_port "$relay_port" && opened="true"
    else
      warn "Could not parse VVIP relay port from $relay_listen; skipped relay firewall auto-open."
    fi
  fi
  is_true "$opened" || warn "No active UFW/firewalld detected; if your VPS has a cloud firewall, allow the main and VVIP relay ports there."
}

configure_firewall_remove() {
  local listen_value="${TOPFLOW_LISTEN:-}"
  [[ -n "$listen_value" ]] || return 0
  local ports=() port relay_listen relay_port
  parse_host_port "$listen_value"
  port="$PARSED_PORT"
  [[ "$port" =~ ^[0-9]+$ ]] && ports+=("$port")
  relay_listen="$(resolve_vvip_relay_listen "$listen_value" "${TOPFLOW_VVIP_RELAY_LISTEN:-off}")"
  if [[ -n "$relay_listen" ]]; then
    parse_host_port "$relay_listen"
    relay_port="$PARSED_PORT"
    [[ "$relay_port" =~ ^[0-9]+$ ]] && ports+=("$relay_port")
  fi
  [[ "${#ports[@]}" -gt 0 ]] || return 0
  if command -v ufw >/dev/null 2>&1 && ufw status 2>/dev/null | grep -qi 'Status: active'; then
    for port in "${ports[@]}"; do ufw delete allow "${port}/tcp" >/dev/null 2>&1 || true; done
  fi
  if command -v firewall-cmd >/dev/null 2>&1 && systemctl is-active --quiet firewalld; then
    for port in "${ports[@]}"; do firewall-cmd --permanent --remove-port="${port}/tcp" >/dev/null 2>&1 || true; done
