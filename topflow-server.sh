...
DEFAULT_DOWNLOAD_URL="https://raw.githubusercontent.com/efrenmotes525/TopFlow/main/headbridge-server"
...
安装参数：
  ...
  --listen <host:port>        监听地址，默认 0.0.0.0:8888
  --public-endpoint <h:p>     给客户端展示的公网地址，例如 1.2.3.4:8443
...
install_dependencies() {
  ...
  if command -v apt-get >/dev/null 2>&1; then
    ...
    DEBIAN_FRONTEND=noninteractive apt-get install -y ca-certificates curl tar gzip openssl qrencode python3 coreutils >/dev/null
  elif command -v dnf >/dev/null 2>&1; then
    dnf install -y ca-certificates curl tar gzip openssl qrencode python3 coreutils shadow-utils >/dev/null
  elif command -v yum >/dev/null 2>&1; then
    yum install -y ca-certificates curl tar gzip openssl qrencode python3 coreutils shadow-utils >/dev/null
  else
    die "暂不支持当前发行版的软件包管理器，请手动安装 curl / openssl / qrencode / python3 / systemd。"
  fi
}
...
detect_public_endpoint() {
  ...
  public_ip="$(curl -4fsSL --max-time 5 https://api.ipify.org 2>/dev/null || true)"
  if [[ -z "$public_ip" ]]; then
    public_ip="$(curl -4fsSL --max-time 5 https://ipv4.icanhazip.com 2>/dev/null | tr -d '\r\n' || true)"
  fi

  if [[ -n "$public_ip" ]]; then
    printf '%s:%s' "$public_ip" "$listen_port"
    return 0
  fi

  printf 'REPLACE_WITH_PUBLIC_IP:%s' "$listen_port"
}
...
EOF
  ...
  if [[ "$host" == "REPLACE_WITH_PUBLIC_IP" ]]; then
    warn "未能自动探测公网 IP，请改用 --public-endpoint your.domain.com:${port} 重新安装，或手动把导入链接里的地址改成真实公网 IP/域名。"
  fi
}
