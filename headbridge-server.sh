...
HEADBRIDGE_ADMIN_TOKEN="$(env_or_default "" HEADBRIDGE_ADMIN_TOKEN TOPFLOW_ADMIN_TOKEN)"
HEADBRIDGE_MAX_CONNECTIONS="$(env_or_default "$DEFAULT_MAX_CONNECTIONS" HEADBRIDGE_MAX_CONNECTIONS TOPFLOW_MAX_CONNECTIONS)"
HEADBRIDGE_VVIP_RELAY_LISTEN="$(env_or_default "auto" HEADBRIDGE_VVIP_RELAY_LISTEN TOPFLOW_VVIP_RELAY_LISTEN)"
...
?????
  ...
  --listen <host:port>      ??????? 0.0.0.0:6379
  --vvip-relay-listen <host:port|auto|off>
                            VVIP 魅影回程监听；默认 auto（主端口+1）
...
?????????
  HEADBRIDGE_PSK / HEADBRIDGE_ADMIN_TOKEN / HEADBRIDGE_LISTEN / HEADBRIDGE_DOWNLOAD_URL / HEADBRIDGE_DEBUG ?

???????
  TOPFLOW_PSK / TOPFLOW_ADMIN_TOKEN / TOPFLOW_LISTEN / TOPFLOW_DOWNLOAD_URL / TOPFLOW_DEBUG ?
EOF
}
...
generate_or_validate_admin_token() {
  ...
  [[ ${#HEADBRIDGE_ADMIN_TOKEN} -ge 16 ]] || die "管理 Token ??????????? 16 ???"
}
...
load_env_if_exists() {
  if [[ -f "$ENV_FILE" ]]; then
    ...
    HEADBRIDGE_ADMIN_TOKEN="${HEADBRIDGE_ADMIN_TOKEN:-${TOPFLOW_ADMIN_TOKEN:-$HEADBRIDGE_ADMIN_TOKEN}}"
    HEADBRIDGE_MAX_CONNECTIONS="${HEADBRIDGE_MAX_CONNECTIONS:-${TOPFLOW_MAX_CONNECTIONS:-$HEADBRIDGE_MAX_CONNECTIONS}}"
    HEADBRIDGE_VVIP_RELAY_LISTEN="${HEADBRIDGE_VVIP_RELAY_LISTEN:-${TOPFLOW_VVIP_RELAY_LISTEN:-$HEADBRIDGE_VVIP_RELAY_LISTEN}}"
...
LISTEN="\${HEADBRIDGE_LISTEN:-\${TOPFLOW_LISTEN:-0.0.0.0:6379}}"
PSK="\${HEADBRIDGE_PSK:-\${TOPFLOW_PSK:-}}"
ADMIN_TOKEN="\${HEADBRIDGE_ADMIN_TOKEN:-\${TOPFLOW_ADMIN_TOKEN:-}}"
MAX_CONNECTIONS="\${HEADBRIDGE_MAX_CONNECTIONS:-\${TOPFLOW_MAX_CONNECTIONS:-10000}}"
VVIP_RELAY_LISTEN="\${HEADBRIDGE_VVIP_RELAY_LISTEN:-\${TOPFLOW_VVIP_RELAY_LISTEN:-auto}}"
...
if [[ -n "\$VVIP_RELAY_LISTEN" && ! "\$VVIP_RELAY_LISTEN" =~ ^(0|false|FALSE|no|NO|off|OFF)$ ]]; then
  if [[ "\$VVIP_RELAY_LISTEN" =~ ^(auto|AUTO)$ ]]; then
    listen_port="\${LISTEN##*:}"
    listen_host="\${LISTEN%:*}"
    if [[ "\$listen_port" =~ ^[0-9]+$ && "\$listen_port" -lt 65535 ]]; then
  ...
  if [[ -n "\$VVIP_RELAY_LISTEN" ]]; then
    args+=(--vvip-relay-listen "\$VVIP_RELAY_LISTEN")
  fi
fi
...
resolve_vvip_relay_listen() {
  ...
  local relay_value="$2"
  [[ -n "$relay_value" ]] || relay_value="auto"
  case "$(lower "$relay_value")" in
    0|false|no|off) return 0 ;;
    auto)
      local listen_port="${listen_value##*:}"
      local listen_host="${listen_value%:*}"
      if [[ "$listen_port" =~ ^[0-9]+$ && "$listen_port" -lt 65535 ]]; then
        printf '%s:%s' "$listen_host" "$((listen_port + 1))"
      fi
      ;;
...
configure_firewall_add() {
  ...
  open_firewall_port "$port" || warn "??????? UFW / firewalld??????????"

  local relay_listen relay_port
  relay_listen="$(resolve_vvip_relay_listen "$HEADBRIDGE_LISTEN" "$HEADBRIDGE_VVIP_RELAY_LISTEN")"
  if [[ -n "$relay_listen" ]]; then
    relay_port="${relay_listen##*:}"
    open_firewall_port "$relay_port" || true
  fi
}
...
configure_firewall_remove() {
  ...
  local relay_listen="${HEADBRIDGE_VVIP_RELAY_LISTEN:-${TOPFLOW_VVIP_RELAY_LISTEN:-auto}}"
  local relay_value relay_port
  relay_value="$(resolve_vvip_relay_listen "$listen_value" "$relay_listen")"
  relay_port="${relay_value##*:}"
  local ports=()
  [[ "$port" =~ ^[0-9]+$ ]] && ports+=("$port")
  [[ -n "$relay_value" && "$relay_port" =~ ^[0-9]+$ ]] && ports+=("$relay_port")
  [[ "${#ports[@]}" -gt 0 ]] || return 0
  if command -v ufw >/dev/null 2>&1 && ufw status 2>/dev/null | grep -qi 'Status: active'; then
...
install_server() {
  ...
  local relay_display
  relay_display="$(resolve_vvip_relay_listen "$HEADBRIDGE_LISTEN" "$HEADBRIDGE_VVIP_RELAY_LISTEN")"
...
VVIP:      $relay_display
PSK:       $HEADBRIDGE_PSK
管理Token: $HEADBRIDGE_ADMIN_TOKEN
???:    $BIN_PATH
????:  $ENV_FILE
????:  $script_path
